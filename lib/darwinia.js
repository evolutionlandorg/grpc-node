...
const account = testKeyring().alice;
// const keypair = new Keyring.default({type: 'ed25519'});
// const account = keypair.addFromSeed(hexToU8a(""));
const endpoint = process.env.CHAIN_WS_ENDPOINT || 'ws://121.199.60.87:9944/';
...
let module_map = {
    "ethBacking": "EthReceiptProof",
    "ethRelay": "EthHeader",
};

async function checkReceipt(param, module, method) {
    const app = async () => {
        try {
            provider = new WsProvider(endpoint);
            let data = JSON.parse(param);
            const proof_record = (0, createType)(registry, module_map[module], data);
            await start();
            const ex = tools.trigger(api.tx, [module, method], [proof_record]);
